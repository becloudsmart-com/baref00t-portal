...
/**
 * Parse the JSON-encoded chip-list emitted by `<MultiEmailInput>`.
 * Returns an empty array on any parse error so a malformed value can't
 * blow up form submission — Zod will then reject any non-email entries.
 */
function parseEmailList(raw: FormDataEntryValue | null): string[] {
  ...
  try {
    ...
    // Fall back to comma-separated parsing for graceful degradation if JS is off.
    return String(raw)
      .split(/[,;\s]+/)
      .map((s) => s.trim())
      .filter(Boolean)
...
const emailListSchema = z.array(z.string().email()).max(50, 'maximum 50 recipients per channel')

const createSchema = z.object({
  name: z.string().min(1, 'name required').max(120),
  tenantId: z.string().regex(guidRegex, 'tenantId must be a GUID'),
  email: z.string().email().or(z.literal('')).optional(),
  consentRecipients: emailListSchema.optional().default([]),
  questionnaireRecipients: emailListSchema.optional().default([]),
  receivers: emailListSchema.optional().default([]),
  emailReportsEnabled: z.boolean().optional().default(true),
...
export async function createCustomerAction(
  ...
  const parsed = createSchema.safeParse({
    ...
    consentRecipients: parseEmailList(formData.get('consentRecipients')),
    questionnaireRecipients: parseEmailList(formData.get('questionnaireRecipients')),
    receivers: parseEmailList(formData.get('receivers')),
...
const updateSchema = z.object({
  ...
  consentRecipients: emailListSchema.optional(),
  questionnaireRecipients: emailListSchema.optional(),
  receivers: emailListSchema.optional(),
