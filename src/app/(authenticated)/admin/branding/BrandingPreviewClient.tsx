...
// (No wrapping needed — Blob URL approach below isolates the iframe
//  from the parent document's CSP completely.)
 ...
 * server-rendered HTML — see CSP-Safe Handler Pattern in CLAUDE.md). The
 * Save button persists branding to the platform, driving outbound emails
 * and reports; portal chrome (nav, favicon) still comes from env vars.
 */
export function BrandingPreviewClient({ initial }: Props) {
  ...
  // Side-by-side layout (form left, preview right) on lg+ — F40. The
  // email/report iframe was previously cropped because we double-wrapped
  // the platform HTML; the F32 srcDoc/blob fix restored the column
  // budget so the 600px-designed templates render comfortably in the
  // right pane on a desktop viewport.
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <Card>
        ...
        <div className="space-y-3">
          ...
          <div>
            <Label htmlFor="b-primary">Brand Colour</Label>
          ...
          <div>
            <Label htmlFor="b-logo">Brand Logo</Label>
            <Input
              id="b-logo"
              value={draft.logoUrl ?? ''}
          ...
          <div>
            <Label htmlFor="b-footer">Email Footer Text</Label>
            <Input
              id="b-footer"
      ...
      <Card>
        ...
        <div className="overflow-hidden rounded-md border border-[color:var(--color-border)] bg-[#f4f4f5]">
          {previews[activeTab] !== undefined ? (
            <iframe
              key={activeTab}
              title={`${activeTab} preview`}
              srcDoc={previews[activeTab]}
              // Match baref00t SaaS — both flags so the report viewer's
              // filter/toc-jump scripts run and inline styles render.
              // The portal's CSP allows 'unsafe-inline' for styles; the
              // iframe content is platform-rendered (not user-input), so
              // the script risk is bounded.
              sandbox="allow-same-origin allow-scripts"
              className={
                activeTab === 'report'
                  ? 'h-[80vh] min-h-[700px] w-full bg-white'
                  : 'h-[70vh] min-h-[600px] w-full bg-white'
              }
            />
          ) : (
        ...
        <p className="mt-2 text-xs text-[color:var(--color-text-muted)]">
          Preview rendered by the platform — exactly the HTML your customer&apos;s mail client
          (or browser, for the report viewer) receives. Save your draft above to update the
          stored values.
        </p>
