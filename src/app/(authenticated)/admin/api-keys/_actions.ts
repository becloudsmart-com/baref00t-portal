...
import { requireAdmin, NotAdminError } from '@/lib/requireAdmin'
...
export async function generateKeyAction(): Promise<CreateKeyResult> {
  try {
    // #985 — a server action is a POST endpoint. Hiding the button in the nav
    // did not stop anyone who could craft the request, and a minted `pk_` key
    // is Admin-equivalent for the partner's whole account.
    await requireAdmin()
    ...
    if (err instanceof NotAdminError) return { ok: false, error: 'Admin role required.' }
...
export async function revokeKeyAction(slot: 1 | 2, force: boolean): Promise<RevokeResult> {
  try {
    // #985 — revoking a key is a denial-of-service against the partner's own
    // integrations, so it is Admin-only for the same reason minting is.
    await requireAdmin()
    if (!force) {
