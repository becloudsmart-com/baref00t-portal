...
// ── F41 Connect — get authorize URL via SDK ────────────────────────────
...
export async function startConnectAction(input: { returnTo: string }): Promise<StartConnectResult> {
  try {
    const result = await partnerClient().mail.startConnect(input)
    return { ok: true, authorizeUrl: result.authorizeUrl }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    logger().warn({ err: msg }, 'startConnectAction failed')
