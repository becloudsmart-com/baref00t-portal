...
/**
 * The signed-in user's Graph access token, or '' when there is none to use.
 *
 * Returns '' when the token is missing or already past its expiry. The jwt
 * callback (`auth.config.ts`) refreshes within 60s of expiry on every
 * `auth()`, so a token that is STILL expired here means the refresh failed
 * (grant revoked) — callers skip the Graph call we know would 401 and fall
 * back to the initials avatar / OIDC claims (#698: never forward a token past
 * its expiresAt).
 */
export function liveGraphToken(
  session: { msAccessToken?: string; msAccessTokenExpiresAt?: number } | null,
): string {
  const token = session?.msAccessToken
  if (!token) return ''
  const expiresAt = session?.msAccessTokenExpiresAt
  if (typeof expiresAt === 'number' && Date.now() / 1000 >= expiresAt) return ''
  return token
}
